  ...
  this.sharedService.getSchoolYears().subscribe({
    next: (res) => {
      if (res.status === 'success') {
       ...
       this.ActiveSchoolYear = this.SchoolYear.find((sy: any) => sy.Status === 'Active') || null;
...
confirmSetActive() {
  ...
  this.sharedService.setActiveSchoolYear(this.selectedSchoolYearID).subscribe({
    next: (res: any) => {
      if (res.status === 'success') {
...
downloadPDFReport() {
  ...
  doc.setFontSize(13);
  doc.text('Activity Logs Report', pageWidth / 2 + 10, 23, { align: 'center' });
  ...
  const dateRange = this.startDate && this.endDate ? `${this.startDate} to ${this.endDate}` : 'All Dates';
...
if (!this.newSectionName || this.newSectionName.trim() === "") {
    ...
    console.log("Section: ",this.newSectionName)
        ...
        this.sharedService.addSection(this.selectedGrade, this.newSectionName)
        .subscribe({
          next: (res) => {
            if (res.status === 'success') {
...
saveEmail() {
  this.sharedService.updateEmail(this.Admin.AccID!, this.Admin.Email!).subscribe({
    next: (res) => {
      if (res.status === 'success') {
...
savePassword() {
  ...
  this.sharedService.updatePassword(this.Admin.AccID!, this.newPassword).subscribe({
  next: (res) =>{
        if(res.status === "success"){
